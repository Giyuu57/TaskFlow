...
const defaultTasks = [
    'Wake up at 6 AM', 'Exercise', 'Pray', 'Study college', 'Written work',
    'Read book', 'Gym', 'Proper diet', 'Study', 'Work'
];
...
function getProgress(dateStr) {
    ...
    const activeDefaultIndices = defaultTasks.map((_, i) => i).filter(i => !deletedDefaults.includes(i));
...
function renderDayView() {
    ...
    if (appData.mood[selectedDate]) {
        document.querySelector(`.mood-btn.${appData.mood[selectedDate]}`).classList.add('active');
    }
    ...
    if (deletedDefaults.length > 0) {
        ...
        resetDiv.innerHTML = `<button class="btn btn-nav" style="font-size: 0.8rem; opacity: 0.7;" onclick="restoreDefaultsForDay()">↺ Restore default tasks for this day</button>`;
        container.appendChild(resetDiv);
    }

    const progress = getProgress(selectedDate);
    const progressPercent = progress.total > 0 ? (progress.completed / progress.total) * 100 : 0;
    document.getElementById('progress-text').textContent = `${progress.completed}/${progress.total} tasks`;
    document.getElementById('progress-fill').style.width = `${progressPercent}%`;

    document.getElementById('notes-textarea').value = appData.notes[selectedDate] || '';
    document.getElementById('reminder-input').value = appData.reminders[selectedDate] || '';
}
...
function deleteDefaultTask(idx) {
    if (!appData.deletedDefaults[selectedDate]) appData.deletedDefaults[selectedDate] = [];
...
function deleteCustomTask(id) {
    appData.customTasks[selectedDate] = (appData.customTasks[selectedDate] || []).filter(t => t.id !== id);
    saveData();
    renderDayView();
}
...
function addCustomTask() {
    ...
    if (text) {
        if (!appData.customTasks[selectedDate]) appData.customTasks[selectedDate] = [];
        appData.customTasks[selectedDate].push({ id: Date.now(), text, completed: false });
        input.value = '';
        saveData();
        renderDayView();
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('reminder-input').addEventListener('blur', (e) => {
        if (selectedDate) {
...
function showView(view) {
    ...
    if (view === 'calendar') {
        ...
        document.querySelectorAll('.btn-nav')[1].classList.add('active');
        renderTracker();
    }
}
...
function toggleDarkMode() {
    ...
    document.getElementById('theme-icon').textContent = appData.darkMode ? '☀️' : '🌙';
    saveData();
}
...
function updateStats() {
    document.getElementById('stat-days').textContent = Object.keys(appData.tasks).length;
    document.getElementById('stat-important').textContent = appData.importantDates.length;
    document.getElementById('stat-goals').textContent = appData.goals.length;
...
function renderTracker() {
    ...
    document.getElementById('goals-progress').textContent = `${completedGoals}/${appData.goals.length}`;
    ...
    document.getElementById('motivational-quote').innerHTML = `<div style="font-size: 2rem; margin-bottom: 1rem;">📈</div>"${quote}"`;
    ...
    for (let i = 6; i >= 0; i--) {
        ...
        barDiv.innerHTML = `
            ...
            <span style="font-size: 0.875rem; font-weight: 600;">${date.toLocaleDateString('en-US', { weekday: 'short' })}</span>
        `;
...
function addGoal() {
    ...
    const text = input.value.trim();
    if (text) {
        appData.goals.push({ id: Date.now(), text, completed: false });
        input.value = '';
        saveData();
        renderTracker();
    }
...
function renderHabits() {
    ...
    appData.habits.forEach(habit => {
        ...
        last7Days.forEach(dateStr => {
            ...
            daysHtml += `<div class="habit-day ${completed ? 'completed' : ''}" onclick="toggleHabitDay(${habit.id}, '${dateStr}')">${day}</div>`;
        });
...
function addHabit() {
    ...
    if (name) {
        appData.habits.push({ id: Date.now(), name, days: {} });
        input.value = '';
        saveData();
        renderTracker();
    }
